...
def calculate_moving_average(datas, window_sizey):
    # Calculate the moving average using the rolling function
    datas['SMA'] = datas['Close'].rolling(window=window_sizey).mean()
    return datas
...
while forever == 1:
    ...
    while mode == 0:
        ...
        stock_data_moving_average = calculate_moving_average(stock_data, window_size)
        try:
            price = get_stock_price(symbol)
            times = datetime.now()

            print(f"Current price of {symbol} is: {price:.3f} at {times} and SMA = {stock_data['SMA'].iloc[-1]:.3f}")
            if (price > stock_data['SMA'].iloc[-1]) and (stock_data['SMA'].iloc[-2] < stock_data['SMA'].iloc[-1]) and \
                    (stock_data['SMA'].iloc[-3] < stock_data['SMA'].iloc[-2]):
                buy()
                mode = 1
                print(f"Buy signal: Current price {price:.3f} > moving average {stock_data['SMA'].iloc[-1]:.3f} "
                ...
                while mode == 1:
                    ...
                    try:
                        ...
                        print(f"Current price of {symbol} is: {price:.3f} at {times} and SMA = "
                              f"{stock_data['SMA'].iloc[-1]:.3f}")
                        if price < stock_data['SMA'].iloc[-1]:
                            sell()
                            mode = 2
                            print(
                                f"sell signal:Current pr {price:.3f} < moving average {stock_data['SMA'].iloc[-1]:.3f} "
                                f"Current Price of {symbol}:")
